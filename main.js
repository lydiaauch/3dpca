...
function scatter(data) {
    ...
    var xScale = d3.scale.linear()
        .domain(d3.extent(unfiltered, function (d) {return d.e1; }))
        .range([-50,50]);
    var yScale = d3.scale.linear()
        .domain(d3.extent(unfiltered, function (d) {return d.e2; }))
        .range([-50,50]);
    var zScale = d3.scale.linear()
        .domain(d3.extent(unfiltered, function (d) {return d.e3; }))
        .range([-50,50]);
    ...
    for (var i = 0; i < pointCount; i ++) {
        var x = xScale(unfiltered[i].e1),
            y = yScale(unfiltered[i].e2),
            z = zScale(unfiltered[i].e3);
    ...
    var LWIDTH = document.getElementById('legend').offsetWidth,
        HEIGHT = document.getElementById('legend').offsetHeight,
        COLORBLOCK = 15,
        SPACING = 4;
    ...
    var svg = d3.select("#legend").append("svg")
        ...
        .attr("height", categories.length * 19 + 5);
