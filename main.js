var scene, camera, renderer, plotContainer;
...
function checkCount() {
    var checked = [];
    var count = 0;
    if(document.getElementById('e1').checked) { count = count + 1; checked.push("e1"); }
    if(document.getElementById('e2').checked) { count = count + 1; checked.push("e2"); }
    if(document.getElementById('e3').checked) { count = count + 1; checked.push("e3"); }
    if(document.getElementById('e4').checked) { count = count + 1; checked.push("e4"); }
    if(document.getElementById('e5').checked) { count = count + 1; checked.push("e5"); }
    if(document.getElementById('e6').checked) { count = count + 1; checked.push("e6"); }
...
function init() {
    ...
    var PWIDTH = plotContainer.offsetWidth,
    ...
    window.addEventListener('resize', function() {
        var PWIDTH = plotContainer.offsetWidth,
            HEIGHT = plotContainer.offsetHeight,
            LWIDTH = document.getElementById('legend').offsetWidth;
...
function scatter(data) {
    ...
    var legend = svg.selectAll(".legend")
        ...
        .on("click", function(name){
            clicked = this;
            change = get_all(name, points); // array of Mesh's
    ...
    var on_button = document.getElementById("toggle_on");
    on_button.onclick = function () {
        array = points.children;
        d3.selectAll(".ldata").select("rect").style("fill", function(i) {
            return color(i); });
        array.forEach(function(i){
            i.material.color.setStyle(color(i.name));
            i.material.visible = true;
            i.state = "on"
        })
    };

    var gray_button = document.getElementById("toggle_gray");
    ...
    var off_button = document.getElementById("toggle_off");
